...
public interface ItemPhoneMapper {
	...
	@Select("SELECT COUNT(*) FROM item_phone p JOIN member m ON p.mem_num = m.mem_num")
	...
	@Select("SELECT * FROM item_phone p JOIN member m ON p.mem_num=m.mem_num WHERE p.phone_num=#{phone_num}")
