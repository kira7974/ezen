...
<script type="text/javascript">
	$(document).ready(function(){
				...
				success:function(data){
					...
					if(count < 0 || list == null){
						...
						$(list).each(function(index,item){
							...
							output += '    <p>' + item.qa_comm_content.replace(/</gi,'&lt;').replace(/>/gi,'&gt;') + '</p>';
							output += item.qa_comm_date;
							
							if($('#mem_num').val()==item.mem_num){
								//로그인 한 회원 번호가 댓글 작성자 번호와 같으면
								output += '  <input type="button" data-num="'+item.qa_comm_num+'" data-mem="'+item.mem_num+'" value="수정" class="modify-btn">';
								output += '  <input type="button" data-num="'+item.qa_comm_num+'" data-mem="'+item.mem_num+'" value="삭제" class="delete-btn">';
							}
		...
		$('#reply_form').submit(function(event){
			if($('#content').val()==''){
				alert('내용을 입력하세요');
				$('#qa_comm_content').focus();
				return false;
			}
			
			$.ajax({
				url:'commentQAWrite.do',
				type:'post',
				data:{qa_num:$('#qa_num').val(),qa_comm_content:$('#qa_comm_content').val()},
				dataType:'json',
				cache:false,
				timeout:30000,
				success:function(data){
		...
		$(document).on('click','.modify-btn',function(){
			...
			var modifyUI = '<form id="mre_form">'
				modifyUI += '   <input type="hidden" name="qa_comm_num" id="mre_num" value="'+re_num+'">';
				modifyUI += '   <input type="hidden" name="mem_num" id="mmem_num" value="'+mem_num+'">';
				modifyUI += '   <textarea rows="3" cols="50" name="qa_comm_content" id="mre_content" class="rep-content">'+content+'</textarea>';
		...
		$(document).on('click','.delete-btn',function(){
			...
			$.ajax({
				type:'post',
				url:'deleteReply.do',
				data:{qa_comm_num:re_num,mem_num:mem_num},
				dataType:'json',
				cache:false,
				timeout:30000,
				success:function(data){
...
<div class="page-main-style">
	...
	<div class="page-main-style">
		<form method="post" class="coment_width" id="reply_form">
		    <input type="hidden" id="qa_num" value="${qa.qa_num}">
