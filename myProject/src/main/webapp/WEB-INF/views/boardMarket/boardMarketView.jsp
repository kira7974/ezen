...
$(document).ready(function(){
			...
			success:function(data){
				...
				if(count < 0 || list == null){
					...
					$(list).each(function(index,item){
						...
						output += '    <p>' + item.market_comm_content.replace(/</gi,'&lt;').replace(/>/gi,'&gt;') + '</p>';
						output += item.market_comm_date;
						
						if($('#mem_num').val()==item.mem_num){
							//로그인 한 회원 번호가 댓글 작성자 번호와 같으면
							output += '  <input type="button" data-num="'+item.market_comm_num+'" data-mem="'+item.mem_num+'" value="수정" class="modify-btn">';
							output += '  <input type="button" data-num="'+item.market_comm_num+'" data-mem="'+item.mem_num+'" value="삭제" class="delete-btn">';
						}
	...
	$('#reply_form').submit(function(event){
		if($('#content').val()==''){
			alert('내용을 입력하세요');
			$('#market_comm_content').focus();
			return false;
		}
		
		$.ajax({
			url:'commentMarketWrite.do',
			type:'post',
			data:{market_num:$('#market_num').val(),market_comm_content:$('#market_comm_content').val()},
			dataType:'json',
			cache:false,
			timeout:30000,
			success:function(data){
	...
	$(document).on('click','.delete-btn',function(){
		...
		$.ajax({
			type:'post',
			url:'deleteReply.do',
			data:{market_comm_num:re_num,mem_num:mem_num},
			dataType:'json',
			cache:false,
			timeout:30000,
			success:function(data){
	...
	//찜하기 관련
	var checkId = 0;	//1이면 찜하기를 한 걸로 간주
	...
	$('#like_btn').click(function(){
		$.ajax({
			url: 'likeInput.do',
			type: 'post',
			data: {mem_num: $('#mem_num').val(),market_num:$('#market_num').val()},
			dataType: 'json',
			cache: false,
			timeout: 30000,
			success: function(data){
...
<div class="page-main-style">
	...
	<div class="align-right view_btn_modi_del">
		...
		like_btn.onclick=function(){
			var choice = window.confirm('찜하시겠습니까?');
			if(choice) {
				location.href='likeInput.do?market_num=${market.market_num}';
			}
		};	
		</script>            -->        
		</c:if>
	...
	<div class="page-main-style">
		<form method="post" class="coment_width" id="reply_form">
		    <input type="hidden" id="market_num" value="${market.market_num}">
		    ...
		    <ul>
				...
				<li>
					<textarea rows="5" cols="100" id="market_comm_content" class="form-control"></textarea>
				</li>
		    </ul>
